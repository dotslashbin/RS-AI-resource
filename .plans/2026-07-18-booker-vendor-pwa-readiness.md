...
## Notes
- **⬜ TODO (added 2026-09-14): booker's `public/sw.js` caches every same-origin GET, not only static assets.** The fetch handler both apps share said "static assets only", but it cache-firsted RSC payloads (`?_rsc=`) and `/api` GETs forever. Vendor fixed this in `.plans/2026-09-14-vendor-kiosk-next-customer-reset.md` I3: cache-first only for `/_next/static/`, and `OFFLINE_CACHE` bumped to `offline-v2` so `activate` evicts the old entries. The upgrade was verified on a production build. **Booker still has the old handler** (same logic, CRLF line endings). It was not changed because it is a different app. Before copying the fix, check booker's same-origin GETs (`app/api/**` GET routes) and soft navigations with repeating URLs. `architecture/portals.md`'s booker PWA paragraph still describes the old wording.
  **Update 2026-09-19:** Command (which gained the same worker on 2026-08-18) had the identical flaw and worse exposure — `/api/vendor-payout` returns decrypted bank details, which were cached and re-served without writing the payout view audit. **Fixed** with vendor's rule plus `offline-v2` (`.plans/2026-09-17-affiliate-referral-codes-interim.md` K15). **Booker is now the only app still on the unrestricted handler** — latent while it has no GET API routes; fix it before adding one.
