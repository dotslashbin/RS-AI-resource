...
> **Note on the brand name.** The real brand is **Ezzy**. All three Next apps — `booker`, `command`, `vendor` — render it live via `NEXT_PUBLIC_APP_NAME`/`NEXT_PUBLIC_APP_DOMAIN` env vars, re-exported as `APP_NAME`/`APP_DOMAIN` from each app's `lib/constants.ts`. The earlier "Bookdeck" placeholder is fully removed from **application source** in all three apps. It is **not** gone from the database seed: `backbone/supabase/seed.sql` still contains 16 distinct `@bookdeck.com` dev-login addresses across 36 textual occurrences — each account appears twice, once in the header comment table and once in the `insert into auth.users` block (plus two scratch files under `backbone/supabase/snippets/`), tracked as D3 in `.plans/2026-07-14-app-name-env-var.md` and still outstanding. Those are mock credentials for local dev, not a user-facing brand surface. **`NEXT_PUBLIC_APP_NAME` holds the complete per-app product name** ("Ezzy Command", "Ezzy Vendor", "Ezzy booker") — code renders `APP_NAME` verbatim and must never append a portal word to it, which is what produced the "Ezzy Command Command" title fixed on 2026-07-29. Sub-title descriptors under the brand ("Command Portal", "Back Office", "Vendor Portal") are deliberate copy, not app names. The DB portal identifiers `booker` / `vendor` / `command` are separate from the brand and unaffected — the word "Command" in role labels, notification-audience labels, and auth errors refers to those identifiers.
...
An operations dashboard for vendors. Each vendor's admin can define what services they offer, set up recurring or one-time schedule slots, manage their staff, and review and action incoming bookings from bookers — approving or rejecting them, then driving them through fulfilment to the point where the payout is released.
...
An internal ops portal. Command staff approve new vendors (verifying accreditation where applicable), grant or revoke portal access for users, and monitor platform-wide activity. It is the only portal with the authority to change user and vendor status — and, since 2026-08, the only one that can resolve a flagged booking, release a vendor payout, or override a booking's status (with a reason on record).
...
- **Offering duration & booking units (2026-08):** every offering carries a duration as a normalised minute count plus the unit the vendor chose. That unit also decides *how* the offering is booked — minutes/hours mean the booker picks a time of day, days/weeks/months mean whole dates. Bookable slots are **derived** by dividing a schedule's availability window by the offering's duration, never stored. See `schema.md`.
...
| PWA | Web App Manifest + hand-rolled service worker (offline-fallback page on failed navigations; cache-first for fingerprinted `/_next/static/` build output **only** — never `/api` GETs, RSC payloads or cross-origin data) + a dismissible "Install App" banner (real one-tap install on Android/Chromium via `beforeinstallprompt`; instructions-only on iOS Safari, since no equivalent API exists there) | **Booker and vendor** (2026-07), **Command** (2026-08-18). ⚠️ The "static only" rule was not actually enforced in the original worker: vendor was narrowed on 2026-09-14 and Command on 2026-09-19 (it had been caching decrypted payout details — `portals.md`, Command → Installable PWA). **Booker's worker still has the unrestricted version**; it is latent only because booker has no GET API routes. See `.plans/2026-07-18-booker-vendor-pwa-readiness.md` |
...
| Portal | Status |
|--------|--------|
| Ezzy Booker (booker) | Core booking wizard fully functional and Supabase-wired (Steps 1–6). PayMongo Checkout Sessions integrated — booking creates a payment session and redirects the booker to PayMongo's hosted page; webhook sets `is_paid` on return. Dashboard shows booking history fetched from DB, with **booking status updating live** (Realtime, no refresh) when a vendor confirms/rejects/cancels. "My Results" section shows completed bookings as individual cards. In-app notifications live (bell icon, panel, Realtime delivery, arrival toast). Transactions page wired to real bookings (Total Spent / Bookings / Pending summary + payment history). **Booker-side fulfilment shipped 2026-08** — "Yes, all done" / "I've returned it" and flagging, via the `acknowledge_booking()` and `raise_booking_dispute()` RPCs, which are the booker's only write paths to a booking's status. Installable as a PWA (manifest, icons, offline fallback, install banner); the standalone-mode PayMongo checkout round-trip is not yet live-tested on real devices (see `.plans/2026-07-18-booker-vendor-pwa-readiness.md`). |
| Ezzy Vendor (vendor) | Offerings, schedules, staff, profile, and bookings management (approve/reject) are Supabase-wired, with **incoming bookings and status/payment changes appearing live** (Realtime, no refresh). In-app notifications live (arrival toast). Vendor onboarding runs a required **KYC stage** (applicant type → documents → ID + selfie capture) with a private Storage bucket and Command review — no account is created until KYC is submitted (see `vendor-kyc.md`). Installable as a PWA (manifest, icons, offline fallback, install banner). **Full vendor-side booking fulfilment shipped 2026-08** — approve/reject plus hand over / mark as done / got it back / undo / flag across all nine statuses, with the bookings list grouped into six lifecycle filters and the payout gated on mutual completion. A Transactions page reads the real payout ledger. **Kiosk Mode shipped 2026-08-29** — a customer-facing self-service booking surface at `/kiosk` with offering photos, documents and signatures; since 2026-09-12 uploaded documents can be opened at the kiosk and the idle reset pauses while one is being read (`booking-flow.md` → Kiosk Mode). **Bookings (2026-09-12):** search (including by booking-reference prefix), a short-code filter, latest-first default, and a booking details modal showing contact, reference, payment/payout and kiosk agreements with the same actions as the list. Packages are still mock; the calendar reads real bookings and schedules (*this row said "calendar still mock" until 2026-09-12 — `portals.md`'s status table already recorded it as Supabase-wired*). There is **no** Wallet page (an earlier revision of this document claimed one — see `portals.md`). |
| Ezzy Vendor Mobile (ezzy-vendor-mobile) | **Ph0–Ph6 code complete** (2026-07-28): sign-in, password recovery via deep link, session persistence in the OS keystore, vendor access gate + multi-vendor picker, blocked states, tab shell with theme override, dashboard stats, bookings list + approve/reject with a deferred-commit undo, transactions summary + search, in-app notifications with Realtime and an unread badge. **Full vendor-side booking fulfilment shipped 2026-08-02** — hand over / mark as done / got it back / undo / flag, all nine statuses, six lifecycle filters, an auto-confirm countdown, and a payable rule driven by `booking_transactions.payout_status` rather than booking status (`.plans/2026-08-02-vendor-mobile-fulfilment-sync.md`). **First successful run on a real Android device 2026-07-28** via an EAS preview build against hosted Supabase. Push (Ph7) is client- and schema-complete but has no FCM credentials, so it is untested. Store submission (Ph8) still needs listing assets, a privacy policy and a Play account decision — **brand assets no longer block** (2026-07-30). **Nothing has ever run on iOS.** See `.plans/2026-07-27-ezzy-vendor-mobile-companion.md` |
| Ezzy Booker Mobile (ezzy-booker-mobile) | Scaffold only — no app code |
| Ezzy Command (command) | Users and vendors fully Supabase-wired (create/edit/delete), with a **manual refresh button** on each table to pull in changes made elsewhere (e.g. a new registration) without a page reload. In-app notifications live (arrival toast) with platform-wide Notification Type Settings admin page. **Fulfilment oversight shipped 2026-08** — a Flag Queue that resolves booking disputes, a Payouts page that releases vendor payouts in bulk, and an Overview card surfacing stale `in_progress` bookings (the one status with no auto-complete timer). The separate platform-wide Transactions page and most KPI widgets are still seeded. |
