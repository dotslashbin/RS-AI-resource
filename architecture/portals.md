...
- **Affiliates (2026-09-18).** Any user can hold a referral code — it is a capability, not a role, so their role and portals are untouched (see `auth-and-roles.md`). Managed entirely from this page:
  ...
  - **The detail view** shows an Affiliate badge and an Affiliate section: code, a copyable share link, the referred-vendor count and the list of referred vendors. The share link is built server-side from `PORTAL_URL_VENDOR` (which stays server-only) and returned by `GET /api/affiliates`.
